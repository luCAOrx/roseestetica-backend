...
export default {
  ...
  async cadastrar(request: Request, response: Response, next: NextFunction) {
    try {
      ...
      if (await connection('admins').where({ email }).select('email').first()) { return response.status(400).send({ erro: 'Esse email ja existe.' }); }
  ...
  async atualizarDadosDeLogin(request: Request, response: Response, next: NextFunction) {
    try {
      ...
      if (await connection('admins').where({ email }).select('email').first()) return response.status(400).json({ erro: 'Esse e-mail ja existe.' });
  ...
  async esqueciMinhaSenha(request: Request, response: Response, next: NextFunction) {
    try {
      ...
      if (!admin) return response.status(400).json({ erro: 'Email inválido.' });
  ...
  async atualizarSenha(request: Request, response: Response, next: NextFunction) {
    try {
      ...
      if (token !== admin.token_reset_senha) return response.status(400).json({ erro: 'Token inválido.' });

      const horaDeAgora = new Date();

      if (horaDeAgora > admin.expiracao_reset_senha) return response.status(400).json({ erro: 'O token expirou, gere um novo.' });
  ...
  async cadastrarCliente(request: Request, response: Response, next: NextFunction) {
    try {
      ...
      if (await connection('clientes').where({ cpf }).select('cpf').first()) return response.status(400).send({ erro: 'Esse cpf ja existe.' });

      if (await connection('clientes').where({ email }).select('email').first()) { return response.status(400).send({ erro: 'Esse e-mail ja existe.' }); }
  ...
  async agendarCliente(request: Request, response: Response, next: NextFunction) {
    try {
      ...
      if (await connection('agendamentos').where({ horario_id, data }).select('horario_id').first()) return response.status(400).json({ mensagem: 'Horário indisponível,agende para outro dia/horário.' });
