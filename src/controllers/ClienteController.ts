...
export default {
  ...
  async atualizarSenha(request: Request, response: Response) {
    ...
    if (!cliente) { return response.status(400).json({ EmailError: 'E-mail inválido.' }); }

    if (token !== cliente.token_reset_senha) { return response.status(400).json({ TokenError: 'Token inválido.' }); }

    const dataEhoraDeAgora = new Date();

    if (dataEhoraDeAgora > cliente.expiracao_reset_senha) { return response.status(400).json({ erro: 'O token expirou, gere um novo.' }); }
